...
import com.pucmg.tcc.gcbl.proposta3.clinica.service.RecepcionistaService;
import com.pucmg.tcc.gcbl.proposta3.clinica.util.Constantes;


@Controller
@RequestMapping("/privado/**")
public class RecepcionistaController extends ModelController {
    
    private static Log log = LogFactory.getLog(RecepcionistaController.class);
    
    @Autowired
    private RecepcionistaService modelService;

    @Override
    protected Class<Recepcionista> getModelClass() {
        return Recepcionista.class;
    }    

    // -----------------------------------------------------------------------------------
    
    @RequestMapping(value={"/listar-recepcionista"}, method = RequestMethod.GET)
    public String consultar(Model model){
        List<Recepcionista> itemList = modelService.findAll();
        
        model.addAttribute("itemList", itemList);
        return getViewPath() + "listar";
    }
    
    @RequestMapping(value={"/incluir-recepcionista"}, method = RequestMethod.GET)
    public String inserirForm(Model model){
        model.addAttribute(Constantes.ACAO, Constantes.ACAO_INCLUIR);
        
        model.addAttribute(getModelName(), new Recepcionista());
        return getViewPath() + "incluirForm";
    }
    
    @RequestMapping(value={"/incluir-recepcionista"}, method = RequestMethod.POST)
    public String inserir(@Valid Recepcionista item, BindingResult result, Model model, HttpServletRequest request, Locale locale) {                         
        model.addAttribute(Constantes.ACAO, Constantes.ACAO_INCLUIR);
    ...
    @RequestMapping(value={"/excluir-recepcionista"}, method = RequestMethod.GET)
    ...
    @RequestMapping(value={"/editar-recepcionista"}, method = RequestMethod.GET)
    ...
    @RequestMapping(value={"/editar-recepcionista"}, method = RequestMethod.POST)
    public String editar(@Valid Recepcionista item, BindingResult result, Model model, HttpServletRequest request, Locale locale) {
        String mensagem = "";
        model.addAttribute(Constantes.ACAO, Constantes.ACAO_EDITAR);
